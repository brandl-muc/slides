...
\item instances of \code{Derived} cannot be cast to \code{Base} outside of \code{Derived} as this relationship is also private and hence inaccessible
\end{itemize}
...
int createWidget(Widget const ** const widget)
{
	...
	*widget = Widget{}; // zero initialize
	...
	std::string	const name = getNameFromSomewhere(/* ... */);
	if (name.empty())
	{
...
try
{
	...
	std::string	name = getNameFromSomewhere(/* ... */);
	if (name.empty())
	{
		return 0;
	}
...
\begin{column}{0.45\textwidth}
	...
	\medskip
	\begin{itemize}
	\item still possible to delete a \code{WidgetWrapper} via a pointer to \code{Widget} \\ (but easier to remember the function than the \code{static\_cast})
